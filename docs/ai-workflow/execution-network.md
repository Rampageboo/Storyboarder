...
Read this module when a project runs implementation through more than one agent
context. It carries what the roles share; each role's own duties live in its
role file, which only that agent loads.
...
A new session holds runtime instructions, project instructions, and repository
access, and nothing of the conversation that created it. So the assignment
carries the outcome, scope, checks, and authority itself. Repository and runtime
state are evidence the receiver may read; they do not supply a boundary or an
authority the assignment left out.
...
requests, and coverage gaps travel through the coordinator or the report
recipient the assignment names, and through the task directory.
