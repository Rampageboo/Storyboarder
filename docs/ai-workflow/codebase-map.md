...
agent session reads the map. Record it as an adopted automation, including that
hooks install per clone rather than travelling with the repository, so a second
clone does not silently run without one.
