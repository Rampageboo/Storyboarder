...
class StoryboardBackendService(ExportServiceMixin):
    ...
    def _plugin_context_payload(self, project) -> dict[str, Any]:
        ...
        shots = [self._plugin_shot_payload(project, shot) for shot in project.shots]
        selected_index = next(
        ...
        return {
            ...
            "paths": self._plugin_shot_paths(project, selected_shot) if selected_shot else {},
    ...
    def method_plugin_focus_shot(self, shot_id: str) -> dict[str, Any]:
        ...
        return self.method_plugin_context()
    ...
    def method_plugin_next_shot(self, current_shot_id: str | None = None, auto_add: bool = False) -> dict[str, Any]:
        ...
        return {
            "shot": self._plugin_shot_payload(project, next_shot),
            "created": created is not None,
            "context": self.method_plugin_context(),
        }

    def _plugin_shot_payload(self, project, shot: Shot) -> dict[str, Any]:
        data = app_state._shot_payload(project, shot)
        data.update(self._plugin_shot_health(project, shot))
        return data

    def _plugin_shot_paths(self, project, shot: Shot | None) -> dict[str, str]:
        if shot is None:
            return {}
        ...
        psd = self._plugin_shot_psd_path(project, shot)
        return {
    ...
    def _plugin_shot_psd_path(self, project, shot: Shot) -> Path | None:
    ...
    def _plugin_shot_health(self, project, shot: Shot) -> dict[str, Any]:
        psd = self._plugin_shot_psd_path(project, shot)
        preview = project_manager.resolve_shot_preview_path(project, shot)
        thumb = project_manager.resolve_shot_thumbnail_path(project, shot)
        background = project_manager.get_shot_board_background_path(project, shot)
        ...
        return {
            ...
            "paths": self._plugin_shot_paths(project, shot),
